...
from physicsnemo.curator.core.store import FsspecFileStore, RunIndexedFileStore
...
class DrivAerMLSource(Source[Mesh]):
    ...
    * ``"slices"`` — x/y/z-normal slice planes (VTP); yields multiple
    ...
    def _read_volume(self, index: int) -> Generator[Mesh]:
        ...
        """
        assert isinstance(self._store, RunIndexedFileStore)  # noqa: S101
        run_id = self._store.run_indices[index]

        # Try direct VTU first; fall back to concatenating parts.
        direct_remote = f"{self._store._root_path}/run_{run_id}/volume_{run_id}.vtu"
        try:
            local_path = self._store._ensure_local(direct_remote)
        ...
        for part_idx in range(10):  # DrivAerML uses 2 parts typically
            ...
            remote = f"{self._store._root_path}/run_{run_id}/{part_name}"
            try:
                parts.append(self._store._ensure_local(remote))
    ...
    def _read_slices(self, index: int) -> Generator[Mesh]:
        ...
        #
        # Instead, we build a per-run FsspecFileStore lazily.
    ...
    def _build_slices_store(self) -> RunIndexedFileStore:
        ...
        """
        # Use a RunIndexedFileStore just to discover run indices.
        # We use boundary as the template since it's guaranteed to exist.
        ...
        # Build per-run FsspecFileStore objects for slices/.
