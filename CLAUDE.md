...
- **Python API**: `src/physicsnemo/curator/` — public Python interface
...
The Rust extension is exposed as `physicsnemo.curator._lib` and imported in
`__init__.py`. Type stubs live at `src/physicsnemo/curator/_lib.pyi`.
...
- **add-filter**: Guide for adding a new filter (discovery questions,
  implementation patterns, generator semantics, testing, registration).
