...
The `curator.alch` submodule provides pipeline components for working with
...
```bash
# Install with the alch dependency group
uv sync --group alch
```
...
{class}`~physicsnemo_curator.alch.sources.aselmdb.ASELMDBSource` reads
...
{class}`~physicsnemo_curator.alch.filters.stats.AtomicStatsFilter` computes
...
```python
from physicsnemo_curator.alch.filters.stats import AtomicStatsFilter
...
{func}`~physicsnemo_curator.alch.filters.stats.merge_welford_stats`) combine
shards using Chan's parallel Welford algorithm — producing exact aggregate
statistics without re-reading raw data.

### AtomicDataZarrSink

{class}`~physicsnemo_curator.alch.sinks.zarr_writer.AtomicDataZarrSink`
