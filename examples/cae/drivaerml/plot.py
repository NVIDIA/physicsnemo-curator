...
"""Plot DrivAerML interior slices via direct memmap reads (fast, no DomainMesh load)."""
...
def plot_meshes(
    ...
    for row, pdmsh in enumerate(pdmsh_dirs):
        ...
        # Subsample first to avoid scanning all 137M+ points for the slice
        rng = np.random.default_rng(42)
        sample_size = min(n_points, max_pts * 5)
    ...
    plt.suptitle(
        f"DrivAerML Interior — {slice_axis.upper()}-slice (tol={slice_tol})",
        fontsize=12,
        y=0.98,
    )
    plt.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot DrivAerML interior slices (fast)")
    parser.add_argument("--output", type=Path, default=Path("output/drivaerml"), help="ETL output directory")
    ...
    parser.add_argument("--slice-tol", type=float, default=0.5, help="Slice half-thickness")
